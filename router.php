...
function route($route, $path_to_include){
  ...
  if( $route_parts[0] == '' && count($request_url_parts) == 0 ){
    execute($ROOT, $path_to_include);//edited
    exit();
  }
  ...
  for( $__i__ = 0; $__i__ < count($route_parts); $__i__++ ){
    ...
    if( preg_match("/^[$]/", $route_part) ){
      ...
      $parameters[$route_part] = $request_url_parts[$__i__]; //added
  ...
  execute($ROOT, $path_to_include, $parameters); //edited
  exit();
}
...
//aded
function execute($ROOT, $path_to_include, $params = []){
  if(str_ends_with($path_to_include, '.php')) include_once("$ROOT/$path_to_include");
  else {
    $exe = explode('/', $path_to_include);
    $classToRequire = $exe[0];
    $funtionToExecute = $exe[1];
    require_once ("$ROOT/controllers/$classToRequire.php");
    $class = new $classToRequire();
    $class->$funtionToExecute($params);
  }
}